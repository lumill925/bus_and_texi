public class bus extends transport {

    int busPay = 1000;
    String alert = "최대 승객 수 초과";

    public bus(int transportNumber, int remainGas, int speed, int changeSpeed, int maxBusPassenger, int maxTexiPassenger, int currentPassenger, int addPassenger, boolean drive, String status) {
        super(transportNumber, remainGas, speed, changeSpeed, maxBusPassenger, maxTexiPassenger, currentPassenger, addPassenger, drive, status);
        this.busPay = busPay;
        this.alert = alert;
    }


    public int getbuspay() {
        return busPay * addPassenger;
    }

    public String alert() {
        if(currentPassenger > maxBusPassenger){
            System.out.println(alert);
        }
        return null;
    }

    public boolean getdrive() {
        if (this.remainGas >= 10) {
            System.out.println("기름이 " + remainGas + "남았습니다.");
            System.out.println("운행중입니다.");
            remainGas -= 5;
            return true;
        } else {
            System.out.println("기름이 " + remainGas + "남았습니다.");
            System.out.println("주유가 필요합니다");
            System.out.println("차고지행입니다.");
            status = "차고지행";
            return false;
        }
    }
}

